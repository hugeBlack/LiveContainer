...
static void SecItemGuestHooksInit()  {
    ...
    NSData *data = [containerId dataUsingEncoding:NSUTF8StringEncoding];
    uint8_t hash[CC_SHA1_DIGEST_LENGTH];
    CC_SHA1(data.bytes, (CC_LONG)data.length, hash);
    // we create 256 keychain access groups so no more -25299
    accessGroup = [NSString stringWithFormat:@"%@.com.kdt.livecontainer.shared.%d", groupId, hash[0]];
    NSLog(@"[LC] group = %@", accessGroup);
